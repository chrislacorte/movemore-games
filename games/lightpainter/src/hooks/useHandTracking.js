...
export function useHandTracking(videoElement) {
  ...
  const refreshHand = useCallback((now = performance.now()) => {
    ...
    handsRef.current = detection.hands
  }, [])
  ...
  useEffect(() => {
    ...
    const filterLandmarks = (landmarks, handIdx, now) => {
      return landmarks.map((lm, i) => {
        const filtered = getFilter(`h${handIdx}_lm_${i}`).filter(lm.x, lm.y, now)
        ...
        try {
          ...
          if (rawHands.length) {
            const hands = rawHands
              ...
              .map((hand, handIdx) => filterLandmarks(hand, handIdx, detectTs))
  ...
  return { isReady, handsRef, error, refreshHand, modelStatus }
}
